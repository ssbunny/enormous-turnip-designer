...
ready.reselect = function () {
    $.each($('select'), function (index, value) {
        var sthis = $(this);
        if (!sthis.parents('.' + CLASS_POPUP)[0]) {
            (sthis.attr('popup') == 1 && $('.' + CLASS_POPUP).length < 1) && sthis.removeAttr('popup').show();
        }
        sthis = null;
    });
};
...
Class.pt.IE6 = function (layero) {
    ...
    $('select').each(function (index, value) {
        var sthis = $(this);
        if (!sthis.parents('.' + CLASS_POPUP)[0]) {
            sthis.css('display') === 'none' || sthis.attr({'popup': '1'}).hide();
        }
        sthis = null;
    });
};
...
//相册层
popup.photos = function (options, loop, key) {
    var dict = {};
    options = options || {};
    ...
    if (!type) { //页面直接获取
        var parent = $(options.photos), pushData = function () {
            ...
            parent.find(options.img).each(function (index) {
                var othis = $(this);
                othis.attr('popup-index', index);
                data.push({
                    alt: othis.attr('alt'),
                    pid: othis.attr('popup-pid'),
                    src: othis.attr('popup-src') || othis.attr('src'),
        ...
        loop || parent.on('click', options.img, function () {
            var othis = $(this), index = othis.attr('popup-index');
            popup.photos($.extend(options, {
                photos: {
                    start: index,
                    data: data,
                    tab: options.tab
    ...
    dict.event = function () {
        ...
        dict.bigimg.find('.ssd-popup-imgprev').on('click', function (event) {
            event.preventDefault();
            dict.imgprev();
        });

        dict.bigimg.find('.ssd-popup-imgnext').on('click', function (event) {
            event.preventDefault();
            dict.imgnext();
        });
    ...
    dict.loadi = popup.load(1, {
        shade: 'shade' in options ? false : 0.9,
        scrollbar: false
    });
    loadImage(data[start].src, function (img) {
        popup.close(dict.loadi);
        dict.index = popup.open($.extend({
            type: 1,
            area: function () {
            ...
            shadeClose: true,
            closeBtn: false,
            move: '.ssd-popup-phimg img',
            moveType: 1,
            scrollbar: false,
            moveOut: true,
            anim: Math.random() * 5 | 0,
            skin: 'ssd-popup-photos' + skin('photos'),
            content: '<div class="ssd-popup-phimg">'
            + '<img src="' + data[start].src + '" alt="' + (data[start].alt || '') + '" popup-pid="' + data[start].pid + '">'
            + '<div class="ssd-popup-imgsee">'
            + (data.length > 1 ? '<span class="ssd-popup-imguide"><a href="javascript:;" class="ssd-popup-iconext ssd-popup-imgprev"></a><a href="javascript:;" class="ssd-popup-iconext ssd-popup-imgnext"></a></span>' : '')
            + '<div class="ssd-popup-imgbar" style="display:' + (key ? 'block' : '') + '"><span class="ssd-popup-imgtit"><a href="javascript:;">' + (data[start].alt || '') + '</a><em>' + dict.imgIndex + '/' + data.length + '</em></span></div>'
            + '</div>'
            + '</div>',
            success: function (layero, index) {
                dict.bigimg = layero.find('.ssd-popup-phimg');
                dict.imgsee = layero.find('.ssd-popup-imguide,.ssd-popup-imgbar');
                dict.event(layero);
        ...
        popup.close(dict.loadi);
        popup.msg('&#x5F53;&#x524D;&#x56FE;&#x7247;&#x5730;&#x5740;&#x5F02;&#x5E38;<br>&#x662F;&#x5426;&#x7EE7;&#x7EED;&#x67E5;&#x770B;&#x4E0B;&#x4E00;&#x5F20;&#xFF1F;', {
            time: 30000,
            btn: ['&#x4E0B;&#x4E00;&#x5F20;', '&#x4E0D;&#x770B;&#x4E86;'],
            yes: function () {
